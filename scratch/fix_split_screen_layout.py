...
# 1. Fix line 1276 inline position: relative -> position: absolute
old_onboarding_tag = '<div id="onboarding-screen" style="display: none; position: relative;">'
new_onboarding_tag = '<div id="onboarding-screen" style="display: none; position: absolute; top:0; left:0; right:0; bottom:0; width:100%; height:100%; z-index: 999; flex-direction: column; align-items: center; justify-content: flex-start; padding: 40px 20px; overflow-y: auto; background: var(--bg-gradient);">'

if old_onboarding_tag in content:
    content = content.replace(old_onboarding_tag, new_onboarding_tag)
    print("SUCCESS 1: Fixed #onboarding-screen inline style to position: absolute!")
else:
    print("WARNING 1: Could not find exact old_onboarding_tag.")

# 2. Fix dismissSplashScreen to immediately set splash.style.display = 'none'
old_dismiss_splash = '''if (splash) {
splash.style.opacity = '0';
setTimeout(async () => {
splash.style.display = 'none';'''

new_dismiss_splash = '''if (splash) {
splash.style.opacity = '0';
splash.style.display = 'none';
(async () => {'''

if old_dismiss_splash in content:
    content = content.replace(old_dismiss_splash, new_dismiss_splash)
    print("SUCCESS 2: Updated dismissSplashScreen to hide splash instantly!")
else:
    print("WARNING 2: Could not find exact old_dismiss_splash.")

# 3. Ensure clean auth card formatting in #onboarding-step-path
clean_card_html = '''<!-- ── STEP A: Smart Auth Gateway ──────────────────────────── -->
<div id="onboarding-step-path" class="auth-card" style="text-align: center; width: 100%; max-width: 420px; margin: 20px auto; padding: 28px 20px; background: rgba(18, 14, 46, 0.95); border: 1px solid rgba(232, 200, 121, 0.35); border-radius: 20px; box-shadow: 0 12px 40px rgba(0,0,0,0.6); box-sizing: border-box;">
...
pattern_card = r'<!-- ── STEP A: Smart Auth Gateway ──────────────────────────── -->[\s\S]*?</div>\s*</div>'
m_card = re.search(pattern_card, content)
if m_card:
    content = content.replace(m_card.group(0), clean_card_html)
    print("SUCCESS 3: Updated #onboarding-step-path card layout!")
