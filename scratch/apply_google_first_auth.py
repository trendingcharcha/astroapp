...
# 1-Tap Google First Card HTML for Step Path
google_first_html_1 = '''<!-- ── STEP A: Smart Auth Gateway ──────────────────────────── -->
<div id="onboarding-step-path" class="auth-card" style="text-align: center; width: 100%; padding: 24px 20px; background: rgba(18, 14, 46, 0.95); border: 1px solid rgba(232, 200, 121, 0.35); border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
...
<!-- ⚡ 1-TAP INSTANT GOOGLE AUTH BUTTON (PRIMARY PROMINENT) -->
...
<p style="font-size: 0.72rem; color: #E8C879; margin-bottom: 20px; font-weight: 500;"><span class="k-lbl-en">⚡ Instant setup — No passwords required</span><span class="k-lbl-hi" style="display:none;">⚡ तुरंत सेटअप — पासवर्ड की कोई आवश्यकता नहीं</span></p>

<!-- Optional Email Auth Collapsible -->
<div style="margin-top: 10px; margin-bottom: 16px;">
<button type="button" onclick="const f=document.getElementById('email-auth-form-1'); f.style.display = f.style.display === 'none' ? 'flex' : 'none';" style="background: none; border: none; color: var(--text-muted); font-size: 0.78rem; text-decoration: underline; cursor: pointer; padding: 5px;">
...
<form id="email-auth-form-1" onsubmit="handleEmailAuth(event, 'signin')" style="display:none; flex-direction:column; gap:10px; text-align:left; margin-top: 12px; background: rgba(0,0,0,0.3); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08);">
...
# Replace Card 1
regex1 = r'<!-- ── STEP A: Smart Auth Gateway ──────────────────────────── -->[\s\S]*?</div>\s*</div>'
match1 = re.search(regex1, content)
if match1:
    content = content.replace(match1.group(0), google_first_html_1)
    print("SUCCESS: Replaced Card 1 with 1-Tap Google First UI!")
else:
    print("WARNING: Could not match Card 1 regex.")

# Replace Card 2 (in auth-screen)
google_first_html_2 = '''<div id="auth-screen">
<div class="auth-card" style="text-align: center; width: 100%; padding: 24px 20px; background: rgba(18, 14, 46, 0.95); border: 1px solid rgba(232, 200, 121, 0.35); border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
<div style="margin-bottom: 14px;">
<svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#E8C879" stroke-width="1.8">
<circle cx="12" cy="12" r="10"/>
<path d="M12 2v20M2 12h20"/>
...
<!-- Optional Email Auth Collapsible -->
<div style="margin-top: 10px; margin-bottom: 16px;">
<button type="button" onclick="const f=document.getElementById('email-auth-form-2'); f.style.display = f.style.display === 'none' ? 'flex' : 'none';" style="background: none; border: none; color: var(--text-muted); font-size: 0.78rem; text-decoration: underline; cursor: pointer; padding: 5px;">
<span class="k-lbl-en">Or Sign in with Email / Password ▼</span><span class="k-lbl-hi" style="display:none;">या ईमेल / पासवर्ड से साइन इन करें ▼</span>
</button>

<form id="email-auth-form-2" onsubmit="handleEmailAuth(event, 'signin')" style="display:none; flex-direction:column; gap:10px; text-align:left; margin-top: 12px; background: rgba(0,0,0,0.3); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08);">
<div class="form-group" style="margin:0;">
<input type="email" id="auth-email-2" placeholder="Email address" style="width:100%;">
</div>
<div class="form-group" style="margin:0;">
<input type="password" id="auth-password-2" placeholder="Password (min 6 chars)" style="width:100%;">
</div>
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-top: 4px;">
<button type="button" class="btn" style="padding:10px; font-weight:bold; background: linear-gradient(135deg, #2ecc71, #27ae60); color: #fff;" onclick="handleEmailAuth(event, 'signin')"><span class="k-lbl-en">SIGN IN</span><span class="k-lbl-hi" style="display:none;">साइन इन करें</span></button>
<button type="button" class="btn btn-outline" style="padding:10px; font-weight:bold; border-color: var(--gold); color: var(--gold);" onclick="handleEmailAuth(event, 'signup')"><span class="k-lbl-en">SIGN UP</span><span class="k-lbl-hi" style="display:none;">साइन अप करें</span></button>
...
regex2 = r'<div id="auth-screen">[\s\S]*?<!-- 2\. AUTH SCREEN \(SMART AUTH GATEWAY\) -->[\s\S]*?</div>\s*</div>'
match2 = re.search(regex2, content)
if not match2:
    regex2_simple = r'<div id="auth-screen">[\s\S]*?<!-- Free Guest Preview Button -->[\s\S]*?</button>\s*</div>\s*</div>'
    match2 = re.search(regex2_simple, content)

if match2:
    content = content.replace(match2.group(0), google_first_html_2)
    print("SUCCESS: Replaced Card 2 with 1-Tap Google First UI!")
else:
    print("WARNING: Could not match Card 2 regex.")
